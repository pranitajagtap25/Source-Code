...
function validatecstreg() {
    ...
    var passwordExp = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#])[A-Za-z\d@$!%*?&#]{8,16}$/; // Password validation
    var pincodeExp = /^[0-9]{6}$/; // Variable to validate 6-digit pincode
    ...
    if (document.frmcstreg.customername.value == "") {
        ...
        return false;
    } else if (!document.frmcstreg.pincode.value.match(pincodeExp)) {
        alert("PIN Code should be exactly 6 digits.");
        document.frmcstreg.pincode.focus();
        return false;
        ...
        return false;
    } else if (!document.frmcstreg.password.value.match(passwordExp)) {
        alert("Password must contain 8-16 characters, including at least one uppercase letter, one lowercase letter, one number, and one special character.");
        document.frmcstreg.password.focus();
        return false;
